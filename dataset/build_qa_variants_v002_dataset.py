...
def get_question_words():
    ...
    return "what is the word in the sentence how many words are after first last to from end between and before".split()
...
def build_vocabulary(total_vocab_words: int) -> Tuple[Dict[str, int], Dict[int, str]]:
    ...
    # Base vocabulary from the reference vocab.json
    base_question_words = sorted([
        "after", "are", "first", "how", "in", "is", "many", 
        "sentence", "the", "what", "word", "words"
    ])
    
    # All words required for the new templates
    all_question_words = get_question_words()
    
    # Isolate new words to be added at the very end
    new_words = sorted(list(set(w for w in all_question_words if w not in base_question_words)))
    
    # Numbered words
    words = [f"word{i}" for i in range(1, total_vocab_words + 1)]

    # Assemble the vocabulary in the correct order to preserve existing IDs
    vocab = special_tokens + number_tokens + base_question_words + words + new_words
...
def qa_template_find_word_before_target_word(sentence: List[str]) -> Tuple[str, str]:
    """Q&A for finding the word before a target word."""
    if len(sentence) < 2:
        # Fallback for short sentences
        return "how many words are in the sentence ?", str(len(sentence))
    
    idx = random.randint(1, len(sentence) - 1)
    target_word = sentence[idx]
    question = f"what is the word before {target_word} in the sentence ?"
    answer = sentence[idx - 1]
    return question, answer


def qa_template_count_words_after_word(sentence: List[str]) -> Tuple[str, str]:
    """Q&A for counting words after a specific word."""
    if len(sentence) < 2:
        return qa_template_find_word_before_target_word(sentence)  # Fallback
    
    idx = random.randint(0, len(sentence) - 2)
    target_word = sentence[idx]
    question = f"how many words are after {target_word} in the sentence ?"
    answer = str(len(sentence) - 1 - idx)
    return question, answer


def qa_template_find_last_n_words(sentence: List[str]) -> Tuple[str, str]:
    """Q&A for finding the last N words."""
    if len(sentence) < 2:
        return qa_template_find_word_before_target_word(sentence)  # Fallback
    n = random.randint(2, min(4, len(sentence)))  # Ask for 2, 3, or 4 words
    question = f"what are the last {n} words in the sentence ?"
    answer = " ".join(sentence[-n:])
    return question, answer


def qa_template_count_words_between_two_words(sentence: List[str]) -> Tuple[str, str]:
    """Q&A for counting words between two other words."""
    if len(sentence) < 3:
        # Fallback to a simpler question if not enough words
        return qa_template_find_word_before_target_word(sentence)
    ...
    # This case should be rare with the check above but as a safeguard
    if (idx2 - idx1 - 1) < 0:
        return qa_template_find_word_before_target_word(sentence)

    question = f"how many words are between {word1} and {word2} in the sentence ?"
    answer = str(idx2 - idx1 - 1)
    return question, answer


QA_TEMPLATES = [
    qa_template_find_word_before_target_word,
    qa_template_find_last_n_words,
    qa_template_count_words_between_two_words,
    qa_template_count_words_after_word,
]
