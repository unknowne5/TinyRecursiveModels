"""
build_qa_10_dataset.py - Generate a simple algorithmic Q&A dataset with a tiny vocab.
...
This version uses a tiny vocabulary of 10 words to create the simplest possible task.
"""
...
class QADatasetConfig(BaseModel):
    output_dir: str = "data/simple_qa_10"
    max_seq_len: int = 48  # Max length for padded sequences (Q + Sentence + A)
    num_train_puzzles: int = 5000
    num_test_puzzles: int = 1000
    seed: int = 42
    vocab_size: int = 10  # Number of unique words in the vocabulary
    min_sentence_words: int = 3
    max_sentence_words: int = 5 # Reduced to be less than vocab size
...
def main(config: QADatasetConfig):
    ...
    print("Generating Simple Q&A dataset (10 word vocab)...")
